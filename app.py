...
# سحب المفتاح السري بأمان من إعدادات السيرفر
...
def call_deepseek(prompt):
    ...
    if not API_KEY:
        return "⚠️ خطأ: لم يتم ضبط مفتاح الـ API في إعدادات سيرفر Render (Environment Variables). يرجى إضافته أولاً لتعمل الأدوات حقيقياً."
    ...
    data = {
        "model": "deepseek-chat",  # يمكنك تغييره إلى النموذج المتاح بحسابك
        "messages": [
            {"role": "system", "content": "أنت مساعد أكاديمي خبير ومتخصص في البحث العلمي والمنهجية بالجامعات العربية والعراقية. إجاباتك دقيقة، منظمة، وتستخدم لغة أكاديمية رصينة وخالية من الرموز الزائدة مثل النجوم."},
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"❌ خطأ من خوادم DeepSeek (رمز الخطأ: {response.status_code}). تأكد من شحن حسابك وصحة المفتاح."
...
def tool_search():
    ...
    if request.method == 'POST':
        ...
        # توليد مراجع حقيقية ديناميكية بالذكاء الاصطناعي بناءً على تساؤل المستخدم
        prompt = f"قم بتوليد مرجعين أكاديميين حقيقيين أو مقترحين بدقة عالية جداً يبحثان في موضوع ({query}). لكل مرجع اذكر: العنوان، الأسماء، السنة، اسم المجلة الأكاديمية العربية، وملخص مكثف جداً للبحث."
        ai_response = call_deepseek(prompt)
        
        # وضع الاستجابة الحية لعرضها في الواجهة
        results = [{"ai_data": ai_response}]
